...
def render_table(
    ...
    correctness_rows = [
        ...
        ("Dwell / duplicate", "Suppress before the backend; retain the active view", dwell + duplicate),
        ...
        ("Traffic-class isolation", "Same snapshot may select class-specific candidates", class_isolation),
    ]
    ...
    lines = [
        ...
        "\\begin{table*}[!t]",
        "\\centering",
        "\\caption[Program composition and action-lifecycle correctness]{Program composition and shared-runtime correctness. The standalone core and production ns-3 adapter emit byte-identical canonical rows for all 14 epochs; the function-native F4 study separately measures IR-Load and IR-Class service.}",
    ...
    lines.extend(
        [
            ...
            "\\toprule",
            "Case (standalone / ns-3: \\cmark~/~\\cmark) & Epoch(s) \\\\",
            "\\midrule",
        ]
    )
