...
RUN_ID="${RUN_ID:-eval-v4-parallel-$(date +%Y%m%d-%H%M%S)}"
...
configs=(
  "exp1_action_latency:$MODULE_DIR/utils/wan_sweep_eval_design_v4_exp1_action_latency.json"
  "exp2_burst_collision:$MODULE_DIR/utils/wan_sweep_eval_design_v4_exp2_burst_collision.json"
  "exp2_class_objective:$MODULE_DIR/utils/wan_sweep_eval_design_v4_exp2_class_objective.json"
  "la_ecmp_coverage:$MODULE_DIR/utils/wan_sweep_eval_design_v4_la_ecmp_coverage.json"
)
...
python3 "$MODULE_DIR/utils/merge_wan_sweeps.py" \
  ...
  "$OUT_ROOT/exp1_action_latency-merged" \
  "$OUT_ROOT/exp2_burst_collision-merged" \
  "$OUT_ROOT/exp2_class_objective-merged" \
  "$OUT_ROOT/la_ecmp_coverage-merged"
python3 "$MODULE_DIR/utils/analyze_wan_sweep.py" \
  --input-dir "$OUT_ROOT/all-merged" \
  --output-dir "$OUT_ROOT/all-analysis"

echo "[done] all v4 batches completed"
